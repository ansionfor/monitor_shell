...
function SendDingDing() {
    if [ $2 -eq 1 ]; then
        content=$(echo "[$dingdingKeyword][$hostName] 磁盘告警! 磁盘已使用: $1%, 请尽快处理！")
    elif [ $2 -eq 2 ]; then
        content=$(echo "[$dingdingKeyword][$hostName] CPU告警! CPU已使用: $1%, 请尽快处理！")
    elif [ $2 -eq 3 ]; then
        content=$(echo "[$dingdingKeyword][$hostName] 内存告警! 内存已使用: $1%, 请尽快处理！")
    fi
    echo $content
